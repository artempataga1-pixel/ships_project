...
export function Navbar() {
  ...
  return (
    <>
      ...
      >
        <nav aria-label="Основная навигация" className="max-w-7xl mx-auto px-4 lg:px-6 h-16 flex items-center">
          ...
          <a href="#hero" className="flex items-center gap-2.5 shrink-0">
            ...
            />
            <span className="hidden xl:block text-sm font-semibold text-gold tracking-wide whitespace-nowrap">
              Разумовские и Партнёры
            </span>
          </a>

          {/* Центр: навигация (flex-1 — равные отступы по бокам) */}
          <ul className="hidden xl:flex flex-1 items-center justify-center gap-3 px-4">
            {NAV_LINKS.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
          ...
          <div className="flex items-center gap-3 shrink-0 ml-auto lg:ml-0">
            <span className="hidden xl:block text-xs text-gold font-medium whitespace-nowrap">
              +7 (495) 000-00-00
            </span>
            <a
              href="#contact"
              className="hidden xl:inline-flex items-center px-4 py-2 text-sm font-semibold bg-gold text-charcoal rounded-lg hover:bg-gold-dark transition-all duration-300 whitespace-nowrap"
            >
              Записаться
            </a>
            <button
              onClick={() => setMenuOpen(true)}
              className="xl:hidden p-2 -mr-2 text-white/70 hover:text-white transition-colors"
