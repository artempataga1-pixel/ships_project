...
- **Lighthouse mobile по прод-URL** (`https://project-yuriki.vercel.app/`, после подтверждённого выката новой сборки — старые `practice-*.png`/`infograf*.mp4` на проде вернули 404, новые `.webp` — 200): PageSpeed Insights API вернул `429 RESOURCE_EXHAUSTED` («Quota exceeded... Queries per day») — дневная квота публичного API исчерпана, не факт что моими запросами (общий лимит на IP/проект). Отклонение от плана: вместо PSI прогнал `npx lighthouse` локально (headless Chromium от Playwright, `--form-factor=mobile --screenEmulation.mobile`, симулированный throttling — тот же профиль, что использует PSI, но замер идёт с моей машины, а не из дата-центра Google, так что абсолютные цифры не 1-в-1 сравнимы с «настоящим» PSI). Результат: **Performance = 83** (FCP 1.3s, LCP 3.4s, TBT 290ms, CLS 0.008, Speed Index 4.6s) — **не дотянул до цели ≥85**. Не считаю это блокером шага 3: медиа-цель шага (ноль видео на мобиле, webp вместо PNG) выполнена и проверена; узкое место — TBT/LCP, вероятно упирается в JS-инициализацию (GSAP/ScrollTrigger, устанавливаемые на весь документ ещё до вертикализации секций в шаге 4) и общий вес JS-бандла, а не в медиа. Не героизирую попытками влезть в чужой шаг 4 — сообщаю пользователю явно: цель ≥85 не достигнута на этом шаге, вероятно добьётся в шаге 4 (убрать лишние ScrollTrigger на мобиле) и/или потребует отдельного разбора бандла.
...
**Заметки исполнителя:**
- Реализовано по плану: `PracticesSection.tsx` — пин-обёртка `relative h-dvh overflow-hidden` → `relative lg:h-dvh lg:overflow-hidden`, трек `flex h-full w-max...` → `flex flex-col gap-14 px-6 py-16 lg:h-full lg:w-max lg:flex-row...`, все align/width в `CARD_VARIANTS` и `QuoteBlock` получили `lg:`-префикс (на мобиле карточки full-width вертикальным списком, картинки сохраняют свою природную пропорцию — уже не горизонтальный коллаж, а лента), спейсеры-«коридоры» скрыты на мобиле (`hidden lg:block`), GSAP-пин/скраб/параллакс и reduced-motion-ветка переведены на `mm.add('(min-width:1024px) and ...')` — на <1024 GSAP не создаёт ни одного триггера, `sizes` картинок → `(max-width:1023px) 92vw, 36vw`, заголовочный экран `h-dvh` → `min-h-svh lg:h-dvh`. `ArticlesSection.tsx` — все пиновые `md:`-классы (заголовок, pin-обёртка, декор, `PEEK_STEPS`, `absolute`-позиции карточек, прогресс-бар) переведены на `lg:`, GSAP-пин `min-width:768px` → `min-width:1024px`, `ArticleCard` `min-h-[82dvh] md:min-h-0 md:h-full` → `min-h-0 lg:h-full` (высота по контенту на мобиле), добавлена вторая GSAP-ветка `mm.add('(prefers-reduced-motion:no-preference) and (max-width:1023px)')` — `gsap.from` reveal каждой карточки (`autoAlpha:0, y:40, duration:0.7, power2.out`, `once:true`) без blur, как в плане. `CasesSection.tsx` — `mt-[45dvh]` → `mt-[30dvh] lg:mt-[45dvh]`, sticky-заголовок `h-dvh` → `h-svh lg:h-dvh`; на 360px (`w-[70vw]`≈252px) читаемость проверена скриншотом — сумма/заголовок/«Подробнее→» не теснятся, кегль менять не потребовалось. `ScrollStory.tsx` (`FlowFallback`) — добавлен `data-hero-fade` на 3 строки заголовка, подзаголовок и CTA внутри общего `HeroLayer` (используется и в `StoryScene`, там атрибут ни на что не влияет), новый `useGSAP` со scope на hero-секцию: `mm.add('(prefers-reduced-motion:no-preference)')` → `gsap.set(autoAlpha:0,y:24)` + `gsap.to(autoAlpha:1,y:0,stagger:0.08,duration:0.7,power3.out)`; на reduce-motion ветка не регистрируется — элементы остаются в исходном видимом состоянии (без FOUC, т.к. `gsap.set` синхронный и выполняется только внутри сработавшей ветки). `CompetenciesSection.tsx` — заменил хардкод `px-16` (без брейкпоинтов, 64px паддинг с обеих сторон уже на 360px) на `px-6 md:px-12 lg:px-16`, как у `AboutSection`/`PartnersSection` — на 360px карточки компетенций были неоправданно узкими (~232px), после фикса используют почти всю ширину экрана. `PartnersSection.tsx`/`AboutSection.tsx` — паддинги уже были адаптивны (`px-8 md:px-16` / `px-6 md:px-12 lg:px-16`), числа не менял.
- **Отклонение от плана — найден и исправлен орфанный баг вне списка файлов шага**: `Footer.tsx` (`grid grid-cols-3 gap-12`, без единого брейкпоинта) существовал с коммита `4363e9a8` (2026-07-01, задолго до старта мобильной адаптации) и не был закреплён ни за одним шагом плана (footer вообще не упомянут в шагах 0-6). На 360px это давало горизонтальный оверфлоу document.documentElement.scrollWidth на 27px (3 колонки в ~200px после вычета паддингов) — блокировало собственный критерий проверки шага 4 «нет горизонтального оверфлоу», который применяется ко всей странице, а не только к файлам шага. Бисекцией (поочерёдное `display:none` прямых детей `<body>`) подтвердил: причина именно в Footer, шаг 2 в этом же файле трогал только `pb-[...]` (проверено `git show f6d9520`), к `grid-cols-3` отношения не имеет. Правка изолированная и минимальная: `grid-cols-3 gap-12` → `grid-cols-1 gap-10 sm:grid-cols-3 sm:gap-12`. Overflow после фикса = 0 на 360/390/768.
- **Важная находка для пользователя, НЕ исправлял (вне сферы шага 4 и, вероятно, вне текущего описания шага 5 в плане)**: на странице `/` реально рендерится `ContactsSection.tsx` (+`contacts-section.css`, классы `.pb-contact*`, коммит `7124667` «новый блок «Контакты» из contact-section-package») — а не `ContactForm.tsx`, который называет шаг 5 плана («Файл: `src/components/sections/ContactForm.tsx`»). `ContactForm.tsx` физически существует в репо, но **не импортируется** нигде в `src/app` — то есть сейчас не используется. У реально работающего `ContactsSection.tsx`/`contacts-section.css` при беглой проверке тоже нашёлся independent-от-шага-4 источник переполнения по ширине (honeypot-инпут `.pb-contact__honey`, судя по всему компонент "contact-section-package" не адаптирован под мобильные вьюпорты вообще) — но он полностью изолирован своим собственным `overflow:hidden` на двух уровнях родителей (`label.pb-contact__honey` + `section.pb-contact`), поэтому на итоговый `document.documentElement.scrollWidth` не влияет (проверено бисекцией) и никак не блокирует шаг 4. Раз текст шага 5 плана указывает на файл, который сейчас не подключён к странице — перед началом шага 5 стоит сверить с пользователем, какой из двух файлов актуален, иначе шаг 5 будет править неиспользуемый компонент.
- Проверено Playwright-скриптом `tmp/check-step4-sections.mjs`: 360×800 / 390×844 / 768×1024 — герой анимируется (`data-hero-fade` → opacity>0.9), Практики и Статьи без `.pin-spacer` (вертикальные списки), reveal Статей срабатывает, sticky-заголовок Кейсов на месте (`position:sticky`), после полного прокрута `scrollWidth-clientWidth=0` на всех трёх вьюпортах, консоль чистая. Десктоп 1440 — `.pin-spacer` есть у Практик и Статей (пин жив), консоль чистая; скриншоты интро-экранов Практик и Статей сверены визуально с `tmp/screenshots/mobile-adapt-baseline/02-practices-pinned.png` и `03-articles-pinned.png` — пиксель-в-пиксель совпадение. Планшет 768→1024 (динамический ресайз через `page.setViewportSize`) — пин корректно создаётся после смены брейкпоинта (`gsap.matchMedia` сам реагирует на `MediaQueryList change`; проверка на поллинге до 6с вместо фиксированной паузы — по той же причине нестабильности, что и в шаге 1, первая версия с фиксированным `waitForTimeout(1000)` иногда не укладывалась в тайминг пересоздания триггера под нагрузкой от предыдущих открытых контекстов браузера в том же скрипте). 3 последовательных прогона — стабильно зелёные. `npx tsc --noEmit` — 0 ошибок. `npx eslint .` — те же 2 старые фоновые ошибки в `useStoryController.ts:87,452` (файл не трогал, подтверждены повторно с шагов 0-3).
...
| 3 — Медиа и перф | ✅ | 2026-07-17 | Удалены infograf1-4/dama/poster_end (архив вне репо), practice-1..6 → webp (~30x сжатие, оригиналы в архиве), новый CaseBackground.tsx (постер на <1024, видео на ≥1024) вместо инлайн-video на странице кейса. Playwright (390×844 + 1440×900) зелёный: 0 запросов story*/case-bg-loop на мобиле, video отсутствует в DOM кейса на мобиле, десктоп не сломан. tsc чист, eslint — 2 старые ошибки (фон). Lighthouse mobile (локальный прогон, PSI-квота исчерпана) = 83, цель ≥85 не достигнута — не блокер шага (медиа-часть сделана), вероятно требует шага 4 (GSAP/ScrollTrigger) |
| 4 — Секции | ✅ | 2026-07-17 | Практики/Статьи → вертикальные списки на <1024, пин только lg через gsap.matchMedia; reveal Статей на мобиле; Кейсы/герой — числа величин; Competencies px-16→px-6 md:px-12 lg:px-16. Доп. фикс вне списка файлов: Footer grid-cols-3 (орфанный баг с 2026-07-01, 27px оверфлоу на 360px) → responsive grid. Найдено, НЕ трогал: живой Контакты — ContactsSection.tsx, а не ContactForm.tsx из текста шага 5 (нужна сверка с пользователем перед шагом 5). Playwright 360/390/768/1024+1440 зелёный, десктоп пиксель-в-пиксель с baseline, tsc чист, eslint — 2 старые ошибки (фон) |
| 5 — Форма | ⬜ | | |
| 6 — Полировка + финал | ⬜ | | |
